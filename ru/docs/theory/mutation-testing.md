...
Если просто взять GPT-4 и попросить через API классифицировать мутант, получается **<abbr title="F-мера: гармоническое среднее precision и recall, чем выше — тем лучше">F1</abbr> в районе 53–56%** в зависимости от того, использовать <abbr title="запрос без примеров — модель решает задачу с нуля по самому промпту">zero-shot</abbr> или <abbr title="запрос с несколькими примерами правильных ответов в самом промпте">few-shot</abbr> промптинг. На сбалансированном датасете это близко к подбрасыванию монеты, и для продакшна такой подход пока не годится: «подключить ChatGPT и решить» не работает.

Серьёзные результаты получаются только у специально дообученных моделей: <abbr title="модель, прошедшая дополнительное обучение на узком датасете под конкретную задачу">fine-tuned</abbr> UniXCoder вытаскивает **F1 86.58%** — лучший результат среди десятка проверенных моделей. Это уже неплохо, но за такой результат приходится платить отдельной инфраструктурой: тренировочный пайплайн, размеченные данные, переобучение под конкретный язык. Не «поставил и работает».

Плюс сам бенчмарк сильно синтетический. В нём только Java, доля equivalent искусственно поднята до 17.8%, тогда как в реальных проектах equivalent попадаются скорее эпизодически. То есть даже эти 86.58% — это потолок в благоприятных условиях, а не реалистичная оценка для типичной PHP-кодбазы.

::: tip Из практики на open source
Академические цифры выше — это про cold-prompting и fine-tuning под узкую задачу «классифицируй: equivalent или нет?». В живом проекте задача формулируется иначе: «вот отчёт о мутации, разбери его и почини тесты», и решают её **агентские системы** вроде Claude Code, у которых есть доступ к исходникам, тестам и рантайму. Это уже не «прочитай два листинга и угадай», а полноценный итеративный цикл: посмотреть, запустить, отладить, переписать тест.

Запуск Claude Code на отчётах Infection в самом Testo показал результаты заметно лучше академических — и заметно шире по охвату.

- На первом модуле из **18 выживших мутантов 14 были убиты одним дополнительным ассертом**. Точечного усиления существующих тестов хватило для подавляющего большинства пробелов, и попутно подтянулся code coverage — даже на участках, где он и так был высоким. Из оставшихся четырёх мутантов Claude корректно диагностировал **три как NotCovered из-за бага инструментария** (XDebug с promoted properties — мутации в принципе не достижимы), а четвёртый — как настоящий equivalent.
- На втором модуле Claude нашёл **два бага в смежных компонентах** самого Testo: фильтры работали некорректно, из-за чего нужные тесты не запускались на нужных мутантах. Такие баги вручную ловятся редко — причинно-следственная связь слишком длинная, до неё нужно ещё дойти. Агент сам прошёл по логике фильтра, нашёл root cause и предложил исправление.

В сумме хорошая LLM в роли агента не только убивает «обычных» выживших, но и аккуратно различает три категории, на которых академические бенчмарки традиционно спотыкаются: настоящие equivalent, баги в инструментах вокруг тестов и баги в самом коде. Параллельно с убийством мутантов автоматически растёт и code coverage, что приятно в проекте, где он уже давно «у потолка» и поднимать его руками неинтересно.

Сила здесь приходит не только от удачного промпта — **контекст не менее важен**: доступ к исходникам, к тестам, к отчёту в человекочитаемом виде, к рантайму, в идеале — к истории изменений. Промпт без этого скатывается обратно к академическим 50% F1; контекст без аккуратно сформулированной задачи — это просто файлы в репозитории. Толк получается только на их пересечении.

Всё это снято на OSS, где переписать тест безболезненно и LLM можно дать читать что угодно. На продуктовых проектах с закрытыми политиками подачи кода в LLM — история, конечно, отдельная.
:::
