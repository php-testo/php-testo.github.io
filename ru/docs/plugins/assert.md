...
Testo предоставляет два фасада для проверок в тестах: `Assert` и `Expect`. Оба подключаются автоматически и доступны из коробки.

::: info
За эту функциональность отвечает плагин `\Testo\Assert\AssertPlugin`, входящий в набор `SuitePlugins` по умолчанию.
:::
...
- **Assert** — утверждения. Проверяются здесь и сейчас, на той же строке: «проверил и забыл».
- **Expect** — ожидания. Регистрируются во время теста, а проверяются уже после его завершения: «запомнил, в конце проверил».
