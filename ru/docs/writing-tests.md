...
- Атрибут `#[Test]` из плагина <plugin>Test</plugin> помечает методы и функции как отдельные тесты:
...
- Вместо копирования одного и того же теста для разных данных используйте атрибуты `#[DataSet]` и `#[DataProvider]` из плагина <plugin>Data</plugin>, которые параметризуют тест разными наборами данных:
...
- Атрибут `#[Retry]` из плагина <plugin>Retry</plugin> перезапустит тест при падении, пометив его как нестабильный:
...
Тесты прямо на тестируемом методе с помощью атрибута `#[TestInline]` из плагина <plugin>Inline</plugin> — даже отдельный тестовый класс не нужен:
...
Атрибут `#[Bench]` из плагина <plugin>Bench</plugin> сравнивает производительность функций:
