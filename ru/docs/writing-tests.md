...
|---------------------------------------|---------------------------------------|------------------------------------|
| [Отдельные тесты](#отдельные-тесты)   | <attr>\Testo\Test</attr> / конвенции  | Unit, feature, integration         |
| [Встроенные тесты](#встроенные-тесты) | <attr>\Testo\Inline\TestInline</attr> | Простые проверки в коде приложения |
| [Бенчмарки](#бенчмарки)               | <attr>\Testo\Bench</attr>             | Сравнение производительности       |
...
Для проверок Testo предоставляет два фасада из плагина <plugin>Assert</plugin>:

- <class>\Testo\Assert</class> — утверждения, проверяются здесь и сейчас. Поддерживает цепочки типизированных проверок.
- <class>\Testo\Expect</class> — ожидания, проверяются после завершения теста (исключения, утечки памяти).
...
- Атрибут <attr>\Testo\Test</attr> из плагина <plugin>Test</plugin> помечает методы и функции как отдельные тесты:
...
- Вместо копирования одного и того же теста для разных данных используйте атрибуты <attr>\Testo\Data\DataSet</attr> и <attr>\Testo\Data\DataProvider</attr> из плагина <plugin>Data</plugin>, которые параметризуют тест разными наборами данных:
...
- А вместо <func>\Testo\Expect::exception()</func> можно использовать атрибут <attr>\Testo\Assert\ExpectException</attr>, который просто немного компактнее и добавляет наглядности:
...
- Атрибут <attr>\Testo\Retry</attr> из плагина <plugin>Retry</plugin> перезапустит тест при падении, пометив его как нестабильный:
...
- Хуки жизненного цикла из плагина <plugin>Lifecycle</plugin> помогут подготовить окружение и очистить состояние между тестами:
  ...
  - <attr>\Testo\Lifecycle\BeforeClass</attr> — выполняется один раз перед всеми тестами в классе.
  - <attr>\Testo\Lifecycle\AfterClass</attr> — выполняется один раз после всех тестов в классе.
...
Плагин <plugin>Convention</plugin> находит тесты по паттернам именования — атрибуты не нужны. По умолчанию это суффикс `*Test` на классе и префикс `test*` на методах:
...
Тесты прямо на тестируемом методе с помощью атрибута <attr>\Testo\Inline\TestInline</attr> из плагина <plugin>Inline</plugin> — даже отдельный тестовый класс не нужен:
...
Атрибут <attr>\Testo\Bench</attr> из плагина <plugin>Bench</plugin> сравнивает производительность функций:
...
В модульной архитектуре тесты могут жить в модуле, а конфиги — собираться в один, как в [монорепозитории](configuration.md#монорепозитории).
