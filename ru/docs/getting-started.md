...
Атрибут `#[Test]` помечает метод как тестовый, а фасад `Assert` проверяет утверждения. Подробнее о способах написания тестов, атрибутах и конвенциях — в разделе [Пишем тесты](writing-tests.md).
