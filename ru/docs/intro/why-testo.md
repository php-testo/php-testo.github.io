...
### Полный контроль для разработчика
...
Ядро просто выстраивает несколько pipelines из middleware, а дальше делай что хочешь. Любая фича фреймворка — это middleware или обработчики событий, упакованный в плагин:
- Атрибуты в целом
- Дата-провайдеры
- Inline-тестирование
- Система утверждений (<class>\Testo\Assert</class>, <class>\Testo\Expect</class>)
- CLI рендеринг и TeamCity
- ...

### Продуманное API

Вместо одного раздутого фасада <class>\PHPUnit\Framework\Assert</class>, Testo предоставляет:
- <class>\Testo\Assert</class> — для утверждений (проверяются здесь и сейчас).
- <class>\Testo\Expect</class> — для ожиданий (проверяются после завершения теста).
...
Testo подходит для всех, кто любит PHP.

Open-Source разработчики могут рассчитывать на поддержку [актуальных версий PHP](https://www.php.net/supported-versions.php).
Авторы фреймворков и SDK получат мощный инструмент для создания собственных тестовых сред при интеграции с Testo.

Работяги на проектах получат чистые тесты без необходимости изучать новые синтаксические конструкции или концепции.
Гибкие настройки и система плагинов позволят адаптировать Testo под любые требования проекта, от простых юнит-тестов до сложных интеграционных сценариев.

В конце концов, я сам разработчик фреймворков, SDK и нагруженных проектов, поэтому мне знакомы все эти боли.
