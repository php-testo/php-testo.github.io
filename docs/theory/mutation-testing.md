...
If you simply pick GPT-4 and ask it via API to classify a mutant, you get an **<abbr title="F-measure: the harmonic mean of precision and recall, higher is better">F1</abbr> score in the 53–56% range** depending on whether you use <abbr title="prompting with no examples — the model has to solve the task from the prompt alone">zero-shot</abbr> or <abbr title="prompting with a few worked examples included in the prompt itself">few-shot</abbr> prompting. On a balanced dataset that's close to flipping a coin, and for production it isn't usable yet: "wire up ChatGPT and decide" doesn't work.

Real numbers only show up with specially trained models: a <abbr title="a model that has been further trained on a narrow dataset for a specific task">fine-tuned</abbr> UniXCoder reaches **F1 86.58%** — the best score across the ten models tested. That's a respectable result, but the price is a separate infrastructure: a training pipeline, labelled data, a retraining cycle per language. Not "drop in and run".

On top of that, the benchmark itself is fairly artificial. It's Java only, and the equivalent-mutant ratio is artificially boosted to 17.8% — in real projects, equivalents tend to be sporadic. So even those 86.58% are a ceiling under favourable conditions, not a realistic estimate for a typical PHP codebase.

::: tip From open-source practice
The academic numbers above measure cold prompting and fine-tuning on a narrow task: "classify — is this mutant equivalent or not?". In a real project, the task is framed differently: "here's a mutation report — read it and fix the tests", and it's solved by **agentic systems** like Claude Code that have full access to the source, the tests, and the runtime. That's no longer "read two listings and guess" — it's an iterative loop: look, run, debug, rewrite the test.

Running Claude Code on Infection reports inside Testo itself produced results noticeably better than the academic numbers — and noticeably broader in scope.

- On one module, out of **18 surviving mutants, 14 were killed by adding a single extra assertion**. Tightening the existing tests was enough for the vast majority of the gaps, and as a side effect code coverage went up too — even on stretches where it had already looked high. Of the remaining four, Claude correctly diagnosed **three as NotCovered due to a tooling bug** (an XDebug issue with promoted properties — the mutations literally could not be reached), and identified the fourth as a genuine equivalent.
- On another module, Claude found **two bugs in adjacent components** of Testo itself: the filters were misbehaving, and as a result the right tests weren't actually running against the right mutants. That kind of bug is hard to surface manually — the causal chain is too long to walk through by eye. The agent traced the filter's logic itself, found the root cause, and proposed a fix.

In short, a good LLM in an agentic role doesn't just kill the "ordinary" survivors. It cleanly distinguishes the three categories that academic benchmarks tend to stumble over: genuine equivalents, bugs in the tooling around the tests, and bugs in the source code itself. Code coverage also climbs as a side effect, which is rewarding in a project where it has long been near the ceiling and pushing it higher by hand is a thankless task.

The strength doesn't come from a clever prompt alone — **context matters just as much**: access to the source, to the tests, to the report in human-readable form, to the runtime, ideally to the change history too. A prompt without that drops back to the academic 50% F1; context without a carefully framed task is just files sitting in a repository. The payoff shows up only at the intersection of the two.

All of this was observed on open source, where rewriting a test is painless and you can hand the LLM anything to read. On proprietary projects with strict policies on what code the model can see, the story is, of course, different.
:::
