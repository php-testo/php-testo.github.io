...
- Preview: `public/blog/<page-name>/preview.jpg` → `image: /blog/<page-name>/preview.jpg`.
- List thumbnail: `preview.thumb.jpg` next to the preview. `.vitepress/theme/posts.data.ts` derives the path by swapping the extension for `.thumb.jpg`; thumbnails are generated by `npm run thumbnails` (also run automatically by `npm run build`). If a thumbnail is missing, the blog list falls back to the full preview.
- In-article images live in the same folder (`img-1.png`, `img-2.jpg`, …); locale-specific variants use a language suffix, e.g. `img-2-ru.jpg`.
